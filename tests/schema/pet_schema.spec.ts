...
import { paths, components } from "@schema/openApiType";
import { schemas as autoGeneratedZodSchema } from "@schema/zodSchema";
...
test.beforeAll(async () => {
  const { openapi, version } = await fetchOpenApiInfo(
    "https://petstore31.swagger.io/api/v31/openapi.json"
  );
  console.log("version", openapi, version);
});

test("validate with zod schema", async ({ request }) => {
...
test("validate with Auto Generated zod schema", async ({ request }) => {
  type PetPayloadRequest = components["schemas"]["Pet"];
  type PetResponse =
    paths["/pet"]["post"]["responses"]["200"]["content"]["application/json"];
...
test("validate with auto generated zod schema and mock data", async ({
  request,
}) => {
  const PetSchema = autoGeneratedZodSchema.Pet;
