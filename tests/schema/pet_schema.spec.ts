...
import { paths, components } from 'src/api/types/petStoreType';
import { fetchOpenApiInfo } from 'src/api/versionCheck/fetchOpenApiInfo';
import { schemas as autoGeneratedZodSchema } from 'src/api/schemas/zodSchema';
...
test('validate with zod schema', { tag: '@desktop' }, async ({ request }) => {
...
test('validate with Auto Generated zod schema', { tag: '@desktop' }, async ({ request }) => {
  type PetPayloadRequest = components['schemas']['Pet'];
  type PetResponse = paths['/pet']['post']['responses']['200']['content']['application/json'];
...
test('validate with auto generated zod schema and mock data', { tag: '@desktop' }, async ({ request }) => {
  const PetSchema = autoGeneratedZodSchema.Pet;
